...
const	dap=(Env=>
{
	...
	operate	:{
		...
		"*"	:(value,alias)=>	value && (alias ? value.map(v=>Box(v,alias)) : value),
		     //!value ? false : !alias ? value : Dataset.mapGrid( alias.split(","), value ),
	...
	Compile	= (function(){
		...
		Scope.prototype={
			...
			mods : function(entry){
				let scope=this.entries; 
				while(!(entry in scope.entries); ){
					scope = scope.up || Fail("Entry not defined (for update): "+entry);
				}
			},
			...
			refs : function(entry){
				let scope=this.entries; 
				while(!(entry in scope.entries); ){
					scope.entries[entry]=true;
					scope = scope.up || Fail("Entry not defined (for read): "+entry);
				}
			}
		...
		function Context(ns,branchStack,scope){//,uses,defs
			this.ns	  = ns;
...
//			this.defs = defs;
						...
						const
							...
							lvalues	= a.length>0 && a.map( lvalue=> {
								...
								if(path){						
									if(!tag)
										tag=path.route[0];
									if(path.entry && context.def)
										context.up ? context.scope.mods(path.entry) : context.scope.defs(path.entry);
								}
						...
						if(branches)
							for(let i=branches.length; i-->0;){
								const
									...
									context = rulestring && new Context(this.ns,Parse(rulestring),this.scope),
	...
	Execute	= (function(){
		...
		React	= (ev,target)=>{
			Env.stopEvent(ev);
			Perf(ev.type,Date.now(),Update(target||ev.target,ev));
		};
		...
		Branch.prototype={
			...
			function(snitch,todo){
				...
				const
					...
					value = rule && this.execBranch(todo||rule.todo||rule.engage().todo) || route;
		...
		Func	:{
			...
			operate	:{
				...
				listen : (value,alias,node)=> {
						(value||window).addEventListener(alias,e=>dap.Execute.React(e,node));
					}
			}
		}
	}		
